...
def state(uid, tg_user, user):
    ...
    for cid, chat in user.get(
        ...
        chats.append({
            ...
            "messages": history,
            "requests": int(
                chat.get("requests") or 0
            ),
            "created": int(
                chat.get("created") or 0
            ),
            "last_request": ts,
            "pinned": (
                str(cid) in pinned
    ...
    return {
        ...
        "requests": int(
            user.get("requests") or 0
        ),
        "errors": int(
            user.get("errors") or 0
        ),
        "telegram_user": {
            key: tg_user.get(
                key,
...
def handle_chat(
    ...
    try:
        ...
        if image:
            ...
            name, mime, raw = decode_upload(
                upload
            )

            file_text = bot.read_text_file(
                name,
                raw
            )
...
class MiniAppHandler(
    ...
    def _serve_miniapp(self):
        if not MINIAPP_FILE.exists():
            raise FileNotFoundError(
                f"Mini App file not found: "
                f"{MINIAPP_FILE}"
            )
    ...
    def _do_GET(self):
        ...
        try:
            ...
            if path == "/health":
                send_json(
                    ...
                    200,
                    {
                        ...
                        "version": bot.BOT_VERSION,
                        "port": PORT,
                        "miniapp": MINIAPP_FILE.exists(),
                    }
                )
                return

            if path == "/api/telegram-check":
                ...
                send_json(
                    ...
                    {
                        "ok": True,
                        "initDataReceived": bool(
                            init_data
                        ),
                        "initDataLength": len(
                            init_data
                        ),
                        "userAgent": self.headers.get(
                            "User-Agent",
                            ""
                        ),
                        "telegramWebApp": (
            ...
            if path == "/api/admin/users":
                ...
                for user_id, item in bot.db.get(
                    "users",
                    {}
                ).items():
            ...
            self.error(
                404,
                "Страница или API-метод не найден."
            )
    ...
    def do_GET(self):
        ...
        # Эти страницы/API не требуют Telegram initData.
        if path in (
            "/",
            "/miniapp",
            "/miniapp/",
...
def run_server():
    ...
    print(
        "BulbaMaxAI Mini App server starting...",
        flush=True
    )
    ...
    print(
        f"MINIAPP_FILE: {MINIAPP_FILE}",
        flush=True
    )

    print(
        f"MINIAPP_EXISTS: {MINIAPP_FILE.exists()}",
        flush=True
    )

    print(
        f"BOT_VERSION: {bot.BOT_VERSION}",
        flush=True
    )

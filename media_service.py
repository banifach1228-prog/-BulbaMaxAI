...
from licenses import is_admin, release_request, reserve_request
...
def get_catalog(force=False):
    now = time.time()
    with _catalog_lock:
        if not force and now - _catalog["at"] < CATALOG_TTL and _catalog["items"]:
...
def models_for_kind(kind):
    ...
    return [x for x in get_catalog() if x["kind"] == k]
...
def _request_generate(model, prompt, opts):
    r = _session().post(
        f"{BASE_URL}/api/media/generate",
        headers=_headers(),
        json={"model": model, "prompt": prompt, "opts": opts or {}},
        timeout=35,
    )
    try:
        data = r.json()
    ...
    if not r.ok:
        raise RuntimeError(str(data.get("message") or data.get("error") or f"PlusVibe HTTP {r.status_code}"))
    provider_job = data.get("jobId") or data.get("job_id")
    if provider_job:
        return str(provider_job), data
...
def _release_if_reserved(job_id):
    item = _job(job_id)
    if not item or not item.get("license_reserved") or item.get("license_released"):
        return
    uid = str(item.get("user_id") or "")
    if not uid or is_admin(uid):
        _set_job(job_id, license_released=True)
        return
    ok, _ = release_request(uid)
    if ok:
        _set_job(job_id, license_released=True)
...
def _run_job(job_id, kind, prompt, requested_model, opts):
    try:
        selected = choose_model(kind, requested_model)
        model = selected["id"]
...
def claim_success(job_id):
    """Compatibility guard: successful jobs can be finalized only once."""
    with _jobs_lock:
        item = _jobs.get(str(job_id))

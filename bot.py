...
# Можно переопределить через переменную окружения MINIAPP_URL.
MINIAPP_URL = os.getenv(
    "MINIAPP_URL",
    "https://bot-1788781363-8950-suslik.bothost.tech/"
).strip()
...
def configure_menu_button():
    """
    Автоматически устанавливает Telegram Menu Button
    с открытием Mini App.

    После успешного запуска в логах будет:
    ✅ Telegram Menu Button настроен: ...
    """

    if not MINIAPP_URL:
        print("❌ MINIAPP_URL пустой.")
        return False

    if not MINIAPP_URL.startswith("https://"):
        print("❌ MINIAPP_URL должен начинаться с https://")
        return False
    ...
    try:
        ...
        if result is not None:
            print(
                f"✅ Telegram Menu Button настроен: {MINIAPP_URL}"
            )
            return True

        print("❌ Telegram Menu Button не удалось настроить.")
...
def call_ai(messages, model):
    ...
    Model fallback is handled by ai_chat.
    """
...
def add_global_rule(
    ...
    if mode not in (
        ...
        raise ValueError(
            "mode must be exact, contains or similar"
        )

    rule = {
        "id": int(time.time() * 1000),
        "pattern": str(pattern).strip()[:500],
        "response": str(response).strip()[:4000],
        "mode": mode,
    ...
    for rule in db.get("rules", []):
        if str(rule.get("id")) == str(rule_id):
            ...
            for k in (
                ...
                if (
                    k in changes
                    and changes[k] is not None
                ):
                    rule[k] = changes[k]

            if rule.get("mode") not in (
                "exact",
                "contains",
                "similar",
            ):
                raise ValueError(
                    "Некорректный режим правила."
                )
...
def local_action_plan(
    ...
    """
    Deterministic zero-extra-token router.
    Он никогда не вызывает AI-модель.
    """
    ...
    if (
        not has_image
        and not has_file
        and extract_math(text) is not None
    ):
        return {
            "action": "calculator"
        }
...
def consume_license_after_success(
    user_id
):
    """
    Atomically count a successful licensed request
    after completion.
    """
...
def handle_ai_request(
    ...
    try:
        ...
        if (
            ...
            if not consume_license_after_success(
                user_id or chat_id
            ):
                raise RuntimeError(
                    "Лицензия больше не позволяет выполнить запрос."
                )
        ...
        if action in (
            ...
            if action in (
                ...
                if not answer:
                    raise RuntimeError(
                        error
                        or "Не удалось подготовить содержимое документа."
                    )
...
def _process_message(msg):
    ...
    if text.startswith("/rules"):
        ...
        if not rules:
            ...
            for r in rules:
                lines.append(
                    f"\n• {r.get('id')} | "
                    f"{r.get('mode', r.get('kind', 'contains'))} | "
    ...
    if text.startswith("/rule_add"):
        ...
        if (
            ...
            send_message(
                chat_id,
                "Используй: /rule_add exact|contains|similar ТЕКСТ ОТВЕТ",
                main_keyboard(),
            )
            return
    ...
    if text.startswith("/rule"):
        ...
        if len(parts) < 4:
            send_message(
                chat_id,
                "Используй: /rule contains|exact ТЕКСТ ОТВЕТ",
                main_keyboard(),
            )
            return
    ...
    if text and not msg.get("photo") and not msg.get("document"):
        rule_response = match_global_rule(
            text
        )
    ...
    if text.startswith("/help"):
        send_message(
            ...
            "Команды: /new /clear /stats /memory /remember /forget /models",
            main_keyboard(),
        )
        return
    ...
    if text == "🤖 Авто":
        ...
        send_message(
            chat_id,
            "🤖 Auto включён. Модель и способ обработки выбираются автоматически.",
            main_keyboard(),
        )
    ...
    if "photo" in msg:
        ...
        try:
            ...
            handle_ai_request(
                ...
                u,
                caption
                or "Что изображено на этом фото? Проанализируй изображение.",
                image={
                    "data": data,
                    "mime": "image/jpeg",
                },
    ...
    if "document" in msg:
        ...
        try:
            ...
            handle_ai_request(
                ...
                u,
                caption
                or "Проанализируй этот файл и объясни его содержимое.",
                file_text=file_text,
                file_name=doc.get(
                    "file_name",
                    "file.txt",
...
def main():
    ...
    # Автоматически создаём Telegram Menu Button
    # для запуска Mini App.

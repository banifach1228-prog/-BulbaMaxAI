...
_catalog = {"items": [], "at": 0.0}
_catalog_lock = threading.RLock()
_jobs = {}
_jobs_lock = threading.RLock()
_executor = ThreadPoolExecutor(max_workers=max(2, int(os.getenv("MEDIA_WORKERS", "3"))), thread_name_prefix="bulba-media")
...
def normalize_kind(kind):
    return KIND_ALIASES.get(str(kind or "").strip().lower())
...
def _catalog_items(raw):
    ...
    for item in items if isinstance(items, list) else []:
        ...
        try:
            ...
            price = None
        out.append({"id": mid, "name": str(item.get("name") or mid), "kind": kind,
                    "params": params, "variants": variants, "fromRub": price,
                    "priceUnavailable": bool(item.get("priceUnavailable", False))})
    return out
...
def get_catalog(force=False):
    now = time.time()
    with _catalog_lock:
        if not force and _catalog["items"] and now - _catalog["at"] < CATALOG_TTL:
            return list(_catalog["items"])
    if not configured():
        return []
    try:
        r = _session.get(f"{BASE_URL}/api/media-catalog", headers=_headers(), timeout=20)
        r.raise_for_status()
        items = _catalog_items(r.json())
        with _catalog_lock:
            _catalog["items"], _catalog["at"] = items, now
        return list(items)
...
def models_for_kind(kind):
    ...
    return [x for x in get_catalog() if x["kind"] == kind]


def choose_model(kind, requested=None):
    kind = normalize_kind(kind)
    requested = str(requested or "").strip()
    items = models_for_kind(kind)
    if requested:
        for item in items:
    ...
    if not items:
        raise RuntimeError("PlusVibe не вернул доступные модели для этого типа медиа.")
...
def _request_generate(model, prompt, opts):
    payload = {"model": model, "prompt": prompt, "opts": opts or {}}
    # Important: do not retry this POST after an uncertain network timeout.
    r = _session.post(f"{BASE_URL}/api/media/generate", headers=_headers(), json=payload, timeout=35)
    if r.status_code == 202:
        data = r.json()
        job_id = data.get("jobId") or data.get("job_id")
        if not job_id:
            raise RuntimeError("PlusVibe не вернул jobId.")
        return str(job_id), data
    if not r.ok:
        try:
            detail = r.json()
        except Exception:
            detail = r.text[:500]
        raise RuntimeError(str(detail.get("message") if isinstance(detail, dict) else detail))
    data = r.json()
    job_id = data.get("jobId") or data.get("job_id")
    if job_id:
        return str(job_id), data
    urls = data.get("resultUrls") or data.get("urls") or []
    return None, {"status": "success", "resultUrls": urls, "data": data}
...
def _poll_provider(job_id, timeout):
    ...
    while time.time() < deadline:
        ...
        if not r.ok:
            raise RuntimeError(f"PlusVibe status HTTP {r.status_code}")
...
def _timeout_for_kind(kind):
    return {"image": 120, "video": 660, "tts": 180, "music": 300, "3d": 360}.get(kind, 300)
...
def _normalize_result(kind, model, data):
    ...
    if isinstance(urls, str):
        urls = [urls]
    return {"status": "success", "kind": kind, "model": model, "urls": [str(x) for x in urls if x],
            "text": str(data.get("text") or data.get("transcript") or "")}


def _run_job(job_id, kind, prompt, requested_model, opts):
    try:
        model_item = choose_model(kind, requested_model)
        model = model_item["id"]
        _set_job(job_id, status="submitting", model=model)
        provider_job, immediate = _request_generate(model, prompt, opts)
        if provider_job:
            _set_job(job_id, status="processing", provider_job_id=provider_job)
            result = _poll_provider(provider_job, _timeout_for_kind(kind))
        else:
            result = immediate
        final = _normalize_result(kind, model, result)
        if not final["urls"] and kind != "tts":
            raise RuntimeError("PlusVibe завершил задачу без результата.")
        _set_job(job_id, **final)
    except Exception as exc:
        print("Media job error:", repr(exc))
...
def create_job(kind, prompt, model=None, opts=None, user_id=None):
    ...
    if not configured():
        raise RuntimeError("PLUSVIBE_API_KEY не настроен.")
    prompt = str(prompt or "").strip()
    if not prompt:
        raise ValueError("Опиши, что нужно создать.")
    if len(prompt) > 12000:
        raise ValueError("Описание слишком длинное.")
    # Validate model and catalog before accepting the job.
    selected = choose_model(kind, model)
    job_id = uuid.uuid4().hex
    with _jobs_lock:
        _jobs[job_id] = {"job_id": job_id, "user_id": str(user_id or ""), "kind": kind,
                          "model": selected["id"], "status": "queued", "created_at": int(time.time()),
                          "updated_at": int(time.time())}
    _executor.submit(_run_job, job_id, kind, prompt, model, dict(opts or {}))
    return job_id, selected
...
def cleanup_jobs(max_age=86400):
    ...
    with _jobs_lock:
        for jid, item in list(_jobs.items()):
            if item.get("updated_at", item.get("created_at", 0)) < cutoff:
                _jobs.pop(jid, None)

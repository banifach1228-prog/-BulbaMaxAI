...
AGENT_SYSTEM = """Ты — BulbaMaxAI V14, умный агент внутри Telegram.
Твоя задача — понять намерение пользователя и выбрать лучший способ выполнения.
У тебя есть реальные инструменты, но вызывать их напрямую нельзя: ты возвращаешь JSON-план.

Разрешённые actions:
chat — обычный ответ AI.
calculator — точное вычисление.
create_xlsx — создать Excel из данных.
create_csv — создать CSV из данных.
create_chart — создать график из табличных данных.
analyze_file — анализировать предоставленный файл.
vision — анализировать изображение.
none — если нужно только ответить.

Верни ТОЛЬКО JSON:
{"action":"...", "reason":"кратко", "expression":"...", "rows":[["A","B"],["x",1]], "answer_hint":"..."}

Если задача обычная — action=chat.
Если пользователь явно просит таблицу/Excel/табличный файл — create_xlsx.
Если просит CSV — create_csv.
Если просит график/диаграмму и есть данные — create_chart.
Для простого математического выражения — calculator.
...
def handle_ai_request(chat_id, u, text, image=None, file_text=None, file_name=None, media_ref=None):
    ...
    try:
        ...
        if action in ("create_xlsx", "create_csv", "create_chart"):
            ...
            if len(rows) < 2:
                # Let the model answer instead of pretending a file was made.
                action = "chat"
            else:
